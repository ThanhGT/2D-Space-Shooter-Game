...
void GameState::Enter()
{
	...
	if ((healthDestRect.x < 500 && healthDestRect.x > 300) && (healthDestRect.y < 550 && healthDestRect.y > 350))
	{
		...
		healthDestRect.x = ((healthDestRect.x + 150) > (GameEngine::Instance()->GetScreenWidth() - 66) ? 
			GameEngine::Instance()->GetScreenWidth() - 66 : healthDestRect.x + 150);

		healthDestRect.y = ((healthDestRect.y + 150) > (GameEngine::Instance()->GetScreenHeight() - 66) ? 
			GameEngine::Instance()->GetScreenHeight() - 66 : healthDestRect.y + 150);
	}
...
void GameState::Update()
{
	...
	if (isShieldUp)
	{
		// Keep track of when shield went up and if shield JUST went up
		if (isShieldNew)
		{
			shieldFirstFrame = frameTime;
			isShieldNew = false;
			cout << "Shield finished initializing." << endl;
		}
		// Shield IS up and it is NOT the first frame when the shield is up
		else
		{
			// Turn shields off once 10 seconds have passed
			if (frameTime > shieldFirstFrame + 10)
			{
				isShieldUp = false;
	...
	if (lives <= 0)
	{
		...
		Mix_HaltChannel(15);
		GameEngine::Instance()->GetState()->PushState(new GameOverState(false, score));
		return;
	}
	...
	for (int b = 0; b < (int)player->GetBullets().size(); b++)
	{
		for (int i = 0; i < (int)asteroids.size(); i++)
		{
			...
			if (CircleCollisionTest(bullet->GetX(), bullet->GetY(),
				asteroids[i]->GetX() + 15, asteroids[i]->GetY() + 15,
